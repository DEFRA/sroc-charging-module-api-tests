...
Then('I request to delete the invoice with an unknown invoice ID and I am told its unknown', () => {
  cy.get('@billRun').then((billRun) => {
    ...
    InvoiceEndpoints.delete(billRunId, invoiceId, false).then((response) => {
...
And('I request to delete the invoice for {word} and I am told its not linked to this bill run', (customerRef) => {
  cy.get('@billRun').then((billRun) => {
    const billRunId = billRun.id
    cy.get('@billRun1').then((billRun1) => {
      ...
      BillRunEndpoints.view(billRunId).then((response) => {
        ...
        cy.get('@viewBillRun').then((viewBillRun) => {
          ...
          InvoiceEndpoints.delete(billRunId1, invoiceId, false).then((response) => {
